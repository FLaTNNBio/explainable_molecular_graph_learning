# -*- coding: utf-8 -*-
"""SMILES-MOL-OGBG.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1ZWdioOqiGjpp8JQZ4t5aw7XEYCY34a-Y
"""
...
def smiles2graph(smiles_string, label, removeHs=True, reorder_atoms=False ):
    """
...
def process_SMILES_df(df):
  ...
  for i, row in df.iterrows():
    smile = row['smiles']
    label = row['target']
    graph = smiles2graph(smile,label)
    tensor_g = replace_numpy_with_torchtensor(graph)
    if tensor_g is not None:
      data = Data(edge_index=tensor_g['edge_index'], edge_attr=tensor_g['edge_attr'], x=tensor_g['x'], y=tensor_g['y'], num_nodes = tensor_g['num_nodes'])
      data_list.append(data)
  return data_list


def save_pickle(name_file, data_mol):
  with open('/content/drive/MyDrive/'+ name_file +'.pt', 'wb') as f:
    pickle.dump(data_mol, f)